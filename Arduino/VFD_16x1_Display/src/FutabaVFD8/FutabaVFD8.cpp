...
void VFD::Write(int pos, uint8_t data)
{
    ...
    vfd_send_cmd(0x20 | (pos & 0xF), data);
    vfd_mode_normal();
}
...
void VFD::Printf_P(const char* fmt, ...)
{
    ...
    va_list va;
    va_start(va, fmt);
    int n = vsnprintf_P(buf, 17, fmt, va);
    va_end(va);
...
void VFD::LPrintf_P(int ts, char* fmt, ...)
{
    va_list va;
    ...
    // allocate buffer
    char* buf = (char*) malloc(needed+16);
    memset(buf, 0x20, 16); // Add space to first 16 chars of buffer

    // fill formatted content
    va_start(va, fmt);
    vsnprintf_P(buf+16, needed, fmt, va); // Load chars from Flash to buff in working memory, starting from position 16.
    va_end(va);
...
{
    SetPatterns_P(pos % 8, 5, patterns);
    // Write(pos, pos & 0x7);
    Write(pos, (pos % 8) & 0x7);
    // Write(pos, pos & 0xF);
}

void VFD::Draw_P(int pos, const uint8_t patterns[5], int datapos)
{
    // SetPatterns_P(datapos & 0x7, 5, patterns);
    // Write(pos, datapos & 0x7);
    SetPatterns_P(datapos & 0xF, 5, patterns);
    Write(pos, datapos & 0xF);
}
...
void VFD::Draw(const uint8_t patterns[40], int startpos)
{
    static const PROGMEM byte idx[8] = { 0,1,2,3,4,5,6,7 };
    SetPatterns(0, 40, patterns);
    Write_P(startpos, 8, idx);
}
